...
#define FASTFREE_THRESH 100000
unsigned long check_node_memsize(int node_id) 
{
        struct sysinfo i;
...
is_kloc_obj(void *obj) 
{
	...
	if(obj && current && current->mm && 
		...
		//debug_kloc_obj(obj);
		return 1;
	//}else if(obj && current && current->mm && current->mm->kloc_obj) {
	}else if(obj && current && current->kloc_obj) {
		//dump_stack();
       		//debug_kloc_obj(obj);
        }
	return 0;
