...
int check_valid_hetero_page (struct page *page, struct queue_pages *qp)
{
	int valid = 0;
...
int get_pte_hetero(pmd_t *pmd, unsigned long addr,
                unsigned long end, struct mm_walk *walk)
{
	pte_t *pte = NULL;
	spinlock_t *ptl = NULL;
...
#if 0	
	pte = get_pte_hetero(pmd, addr ,end, walk);
	if(!pte)
		goto release_pte;
#endif
        inode = NULL;
	...
	for_each_possible_cpu(cpu) {
		...
		list_for_each_entry(inode, list_entries, kloc_hlist_entry) {
			...
			struct rb_node *node = NULL;
				...
				if(!check_valid_hetero_page(page, qp))
					continue;
...
static int queue_pages_pte_range_rbtree(pmd_t *pmd, unsigned long addr,
		unsigned long end, struct mm_walk *walk)
{
	struct vm_area_struct *vma = walk->vma;
	struct page *page;
...
#ifdef CONFIG_HETERO_OBJAFF
static int
queue_pages_inactive_lru_hetero(struct mm_struct *mm, unsigned long start, 
		unsigned long end, nodemask_t *nodes, unsigned long flags,
		struct list_head *pagelist, unsigned int *cnt)
{
	struct queue_pages qp = {
		.pagelist = pagelist,
	...
	struct mm_walk queue_pages_walk = {
		.hugetlb_entry = queue_pages_hugetlb,
		.pmd_entry = queue_pages_pte_range_inodelru,
		.test_walk = queue_pages_test_walk,
		.mm = mm,
		.private = &qp,
	};
...
#ifdef CONFIG_HETERO_RBTREE
static int
queue_pages_rbtree_hetero(struct mm_struct *mm, unsigned long start, unsigned long end,
		nodemask_t *nodes, unsigned long flags,
		struct list_head *pagelist, unsigned int *cnt)
{
	struct queue_pages qp = {
	...
	struct mm_walk queue_pages_walk = {
		.hugetlb_entry = queue_pages_hugetlb,
		.pmd_entry = queue_pages_pte_range_rbtree,
		.test_walk = queue_pages_test_walk,
		.mm = mm,
		.private = &qp,
	};
			   ...
			   int flags)
{
	...
	/* 
	 * Steps for Hetero Migration
	 * Step 1 - First migrate pages from inactive inode 
	 * Step 3 - Migrate all pages including application pages 
	 */

#if 0 //def CONFIG_HETERO_OBJAFF
	queue_pages_inactive_lru_hetero(mm, mm->mmap->vm_start, mm->task_size, &nmask,
			flags | MPOL_MF_DISCONTIG_OK, &pagelist, &pagecount);
#endif

#ifdef CONFIG_HETERO_RBTREE
	pagecount = 0;
	/*Step 2 - Next try to migrates pages from both active and inactive inodes */
	//queue_pages_rbtree_hetero(mm, mm->mmap->vm_start, mm->task_size, &nmask,
	//		flags | MPOL_MF_DISCONTIG_OK, &pagelist, &pagecount);

	//printk(KERN_ALERT "%s:%d rbtree_hetero pages added %u \n", 
	...
	/*if((is_hetero_pgcache_set())) {
		printk(KERN_ALERT "%s:%d proc name %s pagecount %d listcount %d\n", 
			__func__,__LINE__, current->comm,  pagecount, page_list_count(&pagelist));
	}*/
