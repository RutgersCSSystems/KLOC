...
int change_vmap_struct(unsigned long addr, struct page *newpage)
{
	...
	if (unlikely(!area)) {
		WARN(1, KERN_ERR "change_vmap_struct Trying to vfree() nonexistent vm area (%lx)\n",
				addr);
		return -1;
	}
...
int migrate_vmalloc_pages(const void *addr, new_page_t get_new_page,
	free_page_t put_new_page, unsigned long private, struct page *src_page)
{
	struct vm_struct *area;
	struct vmap_area *va;
	...
	if (unlikely(atomic_read(&src_page->_mapcount) != -1))
		bad_reason = "nonzero mapcount";
	if (unlikely(src_page->mapping != NULL))
		bad_reason = "non-NULL mapping";
	if (unlikely(page_ref_count(src_page) != 0))
	...
	/* Ensure we are not migrating to the same node */
		...
		unsigned int pagecnt)
{
	...
	/*
	* Check if we do not exceed the threshold for migration 
	*/
	if(!check_listcnt_threshold(pagecnt)) {
		nr_failed++;
		rc = nr_failed;
		return rc;
	}
