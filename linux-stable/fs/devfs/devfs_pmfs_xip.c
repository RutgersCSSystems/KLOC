 ...
 * generic_write_checks() API changes
 * file_remove_suid() -> file_remove_privs()
 *
 */
...
ssize_t pmfs_xip_kernel_read(struct file *filp, char *buf, size_t len, loff_t *ppos)
{
	...
	do {
		...
		memcpy(buf+copied, xip_mem+offset, nr);
		left = 0;
...
//#if 0
static ssize_t
__pmfs_xip_file_read(struct file *filp, char __user *buf, size_t len, loff_t *ppos)
{
	struct address_space *mapping = filp->f_mapping;
	struct inode *inode = mapping->host;
	pgoff_t index, end_index;
			    ...
			    size_t len, loff_t *ppos)
{
	...
	//printk(KERN_ALERT "ERRRRRRRRRRRR \n");
	...
	res = __pmfs_xip_file_read(filp, buf, len, ppos);
	rcu_read_unlock();
	return res;
}
          ...
          size_t len, loff_t *ppos)
{
	...
	iocb.ki_filp = filp;
	iocb.ki_pos = pos;
	iocb.ki_flags = S_ISBLK(inode->i_mode);
	iovi.count = count;
	ret = generic_write_checks(&iocb, &iovi);
	...
	pi = pmfs_get_inode(sb, inode->i_ino);
	//pi = devfss_get_inode(sb, inode->i_ino);
	...
	//offset = pos & (devfs_inode_blk_size(pi) - 1);
				  ...
				  struct vm_fault *vmf)
{
	...
	xip_pfn_t.val = xip_pfn;

	err = vm_insert_mixed(vma, (unsigned long)vmf->address, xip_pfn_t);
				    ...
				    unsigned long addr, unsigned long pgoff)
{
	...
	if (!boot_cpu_has(X86_FEATURE_GBPAGES) || pi->i_blk_type != PMFS_BLOCK_TYPE_1G ||
...
use_2M_mappings:
	...
	if (!boot_cpu_has(X86_FEATURE_PSE) || pi->i_blk_type != PMFS_BLOCK_TYPE_2M ||
