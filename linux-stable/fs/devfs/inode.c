...
extern long devfs_ioctl(struct file *iommu_data, 
...
unsigned int devfs_blk_type_to_shift[DEVFS_BLOCK_TYPE_MAX] = {12, 21, 30};
uint32_t devfs_blk_type_to_size[DEVFS_BLOCK_TYPE_MAX] = {0x1000, 0x200000, 0x40000000};
#endif
...
const struct address_space_operations devfs_aops = {
...
static const struct address_space_operations devfs_aops = {
	...
	.write_begin	= devfs_write_begin,
	.write_end	= devfs_write_end,
	//.writepages     = devfs_writepages,
	//.set_page_dirty	= devfs_set_node_page_dirty,
};
...
const struct file_operations devfs_file_operations = {
        ...
        //.aio_read       = generic_file_aio_read,
        //.write          = new_sync_write,
        //.aio_write      = devfs_file_aio_write,
        //.mmap           = generic_file_mmap,
        .fsync          = noop_fsync,
        //.splice_read    = generic_file_splice_read,
        //.splice_write   = generic_file_splice_write,
                               ...
                               pgoff_t offset, unsigned long req_size)
{
        ...
        /* do read-ahead */
        devfs_ondemand_readahead(mapping, ra, filp, false, offset, req_size);
}
EXPORT_SYMBOL_GPL(page_cache_sync_readahead);
#endif

int do_devfs_open(struct inode *inode, struct file *file)
{
        if (inode->i_private)
 ...
 */
static void devfs_generic_file_read(struct file *filp, loff_t *ppos,
...
find_page:
...
#endif
			page = devfs_find_get_page(mapping, index, ei);
			if (!page) {
		...
		if (PageReadahead(page)) {
			...
			devfs_page_cache_async_readahead(mapping,
					ra, filp, page,
					index, last_index - index);
		}
...
readpage:
		...
		error = devfs_readpage(filp, page);
...
/************************** DevFS IO handler****************************************/
int devfs_readpage(struct file *file, struct page *page)
{
	//dump_stack();
        clear_highpage(page);
        flush_dcache_page(page);
...
int devfs_segment_checks(const struct iovec *iov,
 ...
 */
ssize_t
devfs_file_aio_read(struct kiocb *iocb, const struct iovec *iov,
		unsigned long nr_segs, loff_t pos)
{
	...
	count = 0;
	retval = devfs_segment_checks(iov, &nr_segs, &count, VERIFY_WRITE);
	if (retval)
		return retval;
...
#ifndef _ENABLE_DAX
		devfs_generic_file_read(filp, ppos, &desc, file_read_actor);
#endif
...
EXPORT_SYMBOL(devfs_file_aio_read);
#endif



ssize_t devfs_sync_read(struct file *filp, char __user *buf, size_t len, loff_t *ppos)
{
...
#ifndef _ENABLE_DAX
                ret = devfs_file_aio_read(&kiocb, &iov, 1, kiocb.ki_pos);
#endif
...
EXPORT_SYMBOL(devfs_sync_read);



ssize_t devfs_read(struct file *file, char __user *buf, 
	size_t count, loff_t *pos)
{
	...
	ret = count;
        ...
        if (ret >= 0) {
                count = ret;
                ret = devfs_sync_read(file, buf, count, pos);

				//printk(KERN_ALERT "Reaching %s:%d, res=%d\n",__FUNCTION__,__LINE__,ret);
                ...
                printk(KERN_ALERT "rw_verify_area failed\n");
        }
        return ret;
}
EXPORT_SYMBOL(devfs_read);


ssize_t do_devfs_sync_write(struct file *filp, 
...
#else
        ret = devfs_file_aio_write(&kiocb, &iov, 1, kiocb.ki_pos);
#endif

        *ppos = kiocb.ki_pos;
        return ret;
}
EXPORT_SYMBOL(do_devfs_sync_write);



devfs_transaction_t *devfs_begin_inode_trans(struct super_block *sb, 
		struct inode *inode, struct devfs_inode *ei,  
		unsigned long max_logentries) {

	struct devfss_inode *pi = NULL;
	...
	trans = devfs_new_transaction(ei, MAX_INODE_LENTRIES + max_logentries);
	if (!trans) {
		BUG_ON(!trans);
		return NULL;
	}
	if (devfs_add_logentry(ei, trans, pi, MAX_DATA_PER_LENTRY, LE_DATA)) {
		BUG_ON(trans);
		return NULL;
	}
	return trans;
...
ssize_t devfs_kernel_write(struct file *file, const char *buf, 
		size_t count, loff_t *pos)
{
	...
	trans = devfs_begin_inode_trans(sb, inode, ei, max_logentries);
	if(!trans) {
	...
	ret = do_devfs_sync_write(file, p, count, pos);
#endif
...
EXPORT_SYMBOL(devfs_kernel_write);
 ...
 * devfs fname hash
 */
unsigned int devfs_str_hash_linux(const char *str, unsigned int length)
{
...
unsigned int devfs_str_hash(const char *s, unsigned int len)
{
	return devfs_str_hash_linux(s, len);
}
EXPORT_SYMBOL(devfs_str_hash);


struct devfs_inotree *devfs_inode_list_create(void)
{
        struct devfs_inotree *inotree;
...
EXPORT_SYMBOL(devfs_inode_list_create);



/*add inodes to rbtree node */
//int insert_inode_rbtree(struct devfs_inotree *inotree, struct devfs_inode *ei){
int insert_inode_rbtree(struct rb_root *root, struct devfs_inode *ei){
	...
	devfs_dbgv("%s:%d Before ei->vfs_inode->i_ino\n", 
			__FUNCTION__,__LINE__);
	...
	devfs_dbgv("%s:%d After ei->vfs_inode->i_ino\n", 
			__FUNCTION__,__LINE__);

	/* Figure out where to put new node */
	while (*new) {

		struct devfs_inode *this = rb_entry(*new, struct devfs_inode, rbnode);
		parent = *new;
...
#if !defined(_DEVFS_INODE_OFFLOAD)
		devfs_dbgv("%s:%d After struct devfs_inode *this %lu\n", 
		...
		if (i_ino < this->vfs_inode->i_ino) {
			...
			goto err_insert_inode;
                ...
                devfs_dbgv("%s:%d After rb_entry insert\n", __FUNCTION__,__LINE__);
	}
...
void del_inode_rbtree(struct devfs_inotree *inotree, struct devfs_inode *ei)
{
...
/*find devfs_inode from the inode number */
struct devfs_inode *find_devfs_inode(struct devfs_inotree *inotree, 
				unsigned long i_ino) {
...
EXPORT_SYMBOL(find_devfs_inode);
...
void devfs_final_putname(struct filename *name)
{
...
devfs_getname_flags(const char __user *filename, int flags, int *empty, char *s_kname)
{
...
error:
	devfs_final_putname(result);
	return err;
}

struct filename *
devfs_getname(const char __user * filename, char *s_kname)
{
	return devfs_getname_flags(filename, 0, NULL, s_kname);
}
...
static struct inode *load_inode(struct inode *inode) 
{
	...
	if(inode && inode->devfs_host) {	

		devfs_dbgv("INODE in host memory \n");

		inode = inode_ld_from_host(inode->i_sb, inode);
		BUG_ON(!inode);					
		BUG_ON(inode->devfs_host);
	}
	return inode;
}
...
struct file *devfs_create_file(char *filename, int flags, umode_t mode, int *fd1)
{
	...
	tmp = devfs_getname(fname, filename);
        fd = PTR_ERR(tmp);
...
#if defined(_DEVFS_INODE_OFFLOAD)
				if(f->f_inode && !f->f_inode->devfs_host)
					goto skip_inooff;
...
#if 0
struct file *devfs_create_file(const char *filename, int flags, umode_t mode, int *fd) 
{	
    struct file *fp = NULL; 
...
EXPORT_SYMBOL(devfs_create_file);
#endif

int devfs_permission(struct inode *inode, int mask){
...
ssize_t devfs_perform_write(struct file *file,
...
again:
...
#if defined(_DEVFS_CHECK)
		status = devfs_write_begin(file, mapping, pos, bytes, flags,&page, &fsdata);
#else
		...
		//printk(KERN_ALERT "$$$$ iov_iter: i->type:%d i->iov_offset:%d i->count:%d buf:%lx \n", i->type, i->iov_offset, i->count, i->iov->iov_base);
		//printk(KERN_ALERT "++++++ write pfn: %d\n", page_to_pfn(page)); 
...
#if defined(_DEVFS_CHECK)
		status = devfs_write_end(file, mapping, pos, bytes, copied,
						page, fsdata);
#else
...
EXPORT_SYMBOL(devfs_perform_write);



ssize_t
devfs_file_buffered_write(struct kiocb *iocb, const struct iovec *iov,
...
#ifdef _DEVFS_CHECK
        status = devfs_perform_write(file, &i, pos);
#else
...
EXPORT_SYMBOL(devfs_file_buffered_write);
...
EXPORT_SYMBOL(devfs_segment_checks);
#endif


/**
 * __devfs_file_aio_write - write data to a file
 ...
 */
ssize_t __devfs_file_aio_write(struct kiocb *iocb, const struct iovec *iov,
                                 unsigned long nr_segs, loff_t *ppos)
{
        ...
        ocount = 0;
#if 1 //!defined(_DEVFS_CHECK)
        err = devfs_segment_checks(iov, &nr_segs, &ocount, VERIFY_READ);
        if (err)
                return err;
#else
        nr_segs = 1;
        ...
        if (err)
                goto out;
#endif
	written = devfs_file_buffered_write(iocb, iov, nr_segs,
...
EXPORT_SYMBOL(__devfs_file_aio_write);
 ...
 *
 * This is a wrapper around __devfs_file_aio_write() to be used by most
 * filesystems. It takes care of syncing the file in case of O_SYNC file
 * and acquires i_mutex as needed.
 */
ssize_t devfs_file_aio_write(struct kiocb *iocb, const struct iovec *iov,
                unsigned long nr_segs, loff_t pos)
{
        struct file *file = iocb->ki_filp;
	...
	down_write(&inode->i_rwsem);
        ...
        ret = __devfs_file_aio_write(iocb, iov, nr_segs, &iocb->ki_pos);
...
EXPORT_SYMBOL(devfs_file_aio_write);


#if 1
int devfs_write_begin(struct file *file, struct address_space *mapping,
                        ...
                        struct page **pagep, void **fsdata)
{
	...
	if(page->nvdirty) {
		printk(KERN_ALERT "devfs_write_begin nvdirty set \n");	
	}else {
		//printk(KERN_ALERT "devfs_write_begin nvdirty not set \n");
	}
                        ...
                        struct page **pagep, void **fsdata)
{
	...
	if(ei->isjourn) {
		...
		if(!trans) {
			...
			ei->trans = (devfs_transaction_t *)trans;		
		}
	}
...
#endif
	//page = devfs_get_cache_page(mapping, index, flags, ei, inode);
	page = grab_cache_page_write_begin(mapping, index, flags);
	if (!page)
		return -ENOMEM;
...
#endif
EXPORT_SYMBOL(devfs_write_begin);
 ...
 */
static inline void devfs_i_size_write(struct inode *inode, loff_t i_size)
{
        inode->i_size = i_size;
}

//////////////////In-memory inode/////////////////////////////////////////
inline void devfs_update_nlink(struct inode *inode, struct devfs_inode *ei)
{
        //devfs_memunlock_inode(inode->i_sb, ei);
        ei->i_nlink = cpu_to_le16(inode->i_nlink);
        //devfs_memlock_inode(inode->i_sb, ei);
}

inline void devfs_update_isize(struct inode *inode, struct devfs_inode *ei)
{
        //devfs_memunlock_inode(inode->i_sb, ei);
...
inline void devfs_update_time(struct inode *inode, struct devfs_inode *ei)
{
        //devfs_memunlock_inode(inode->i_sb, ei);
...
int devfs_inode_update(struct inode *inode, loff_t pos){

	struct timespec ts = current_time(inode);
	struct devfs_inode *ei = DEVFS_I(inode);
...
int devfs_set_page_dirty_nobuffers(struct page *page)
{
...
int devfs_set_node_page_dirty(struct page *page)
{
        //struct address_space *mapping = page->mapping;

        SetPageUptodate(page);
        if (!PageDirty(page)) {
                devfs_set_page_dirty_nobuffers(page);
                SetPagePrivate(page);
                return 1;
        }
        return 0;
}
EXPORT_SYMBOL(devfs_set_node_page_dirty);

/**
 * devfs_write_end - .write_end helper for non-block-device FSes
 ...
 * @fsdata:             "
 *
 * devfs_write_end does the minimum needed for updating a page after writing is
 ...
 */
#if 1
int devfs_write_end(struct file *file, struct address_space *mapping,
                        ...
                        struct page *page, void *fsdata)
{
	...
	//printk(KERN_ALERT "%s:%d devfs_write_end \n", 
	//			__FUNCTION__,__LINE__);
                        ...
                        struct page *page, void *fsdata)
{
	...
	if(ei->isjourn) {
		...
		trans = (devfs_transaction_t *)ei->trans;
		if(!trans) {
			printk(KERN_ALERT "%s:%d trans NULL \n",
				__FUNCTION__,__LINE__);
			goto write_err;
...
#if defined(_DEVFS_CHECK)
		i_size_write(inode, last_pos);
		devfs_update_isize(inode, ei);
#else
                i_size_write(inode, last_pos);
#endif
        devfs_update_time(inode, ei);

	if(ei->isjourn)
		devfs_commit_transaction(ei, trans);

        //set_page_dirty(page);
        //devfs_set_node_page_dirty(page);
...
#endif
EXPORT_SYMBOL(devfs_write_end);


/*int devfs_release_file(struct inode *inode, struct file *filp)
{
        return 0;
}*/


static void devfs_get_inode_flags(struct inode *inode, struct devfss_inode *pi)
{
        unsigned int flags = inode->i_flags;
        unsigned int devfs_flags = le32_to_cpu(pi->i_flags);

        devfs_flags &= ~(FS_SYNC_FL | FS_APPEND_FL | FS_IMMUTABLE_FL |
                         FS_NOATIME_FL | FS_DIRSYNC_FL);
        if (flags & S_SYNC)
                devfs_flags |= FS_SYNC_FL;
        if (flags & S_APPEND)
                devfs_flags |= FS_APPEND_FL;
        if (flags & S_IMMUTABLE)
                devfs_flags |= FS_IMMUTABLE_FL;
        if (flags & S_NOATIME)
                devfs_flags |= FS_NOATIME_FL;
        if (flags & S_DIRSYNC)
                devfs_flags |= FS_DIRSYNC_FL;

        pi->i_flags = cpu_to_le32(devfs_flags);
}

#if defined(_DEVFS_DEBUG)
static void devfs_print_inode(struct inode *inode, struct devfss_inode *pi)
{
...
#if 0
static int devfs_update_inode(struct inode *inode, struct devfss_inode *pi)
{
	...
	pi->nova_ino = inode->i_ino;
        devfs_get_inode_flags(inode, pi);
...
struct inode *devfs_get_inode(struct super_block *sb,
	const struct inode *dir, umode_t mode, dev_t dev)
{
...
#endif
	{
		...
		//This should be renamed to devfs_inode_info
		struct devfs_inode *ei;

		u64 ino = devfss_new_devfs_inode(sb, &pi_addr);
		pi = (struct devfss_inode *)devfs_get_block(sb, pi_addr);
		if(!ino || !pi) {
...
#endif
		inode = pmfs_new_inode(NULL, dir, mode, NULL);
#else
		...
		printk("%s: allocating inode %llu, pi %llu, sb %llu piaddr %llu\n",
			__func__, ino, (u64)pi, (u64)devfs_get_super(sb), pi_addr);
#endif

		inode->i_mapping->a_ops = &devfs_aops;
...
#if defined(_DEVFS_NOVA_BASED)
		/*if(devfs_update_inode(inode, pi)){
	        	printk("%s:%d devfs_update_inode failed \n",
				__FUNCTION__,__LINE__);
			return NULL;
		}
		ei = DEVFS_I(inode);
		if(!ei) 
	        	printk("%s:%d finding devfs_inode failed \n",
				__FUNCTION__,__LINE__);
		ei->pi_addr = pi_addr;*/
		//devfs_print_inode(inode, pi);
 ...
 */
/* SMP-safe */
int devfs_mknod(struct inode *dir, struct dentry *dentry, 
	umode_t mode, dev_t dev)
{
	int error = -ENOSPC;
	struct inode *inode = NULL;
	struct super_block *sb = dir->i_sb;
	struct devfs_sb_info *i_sb = DEVFS_SB(sb);
	struct devfs_inode *ei, *eidir;
	...
	inode = devfs_get_inode(sb, dir, mode, dev);
	if(!inode) {	
		printk(KERN_ALERT "%s:%d devfs_get_inode NULL \n",
			 __FUNCTION__,__LINE__);
		goto mknod_err;		
	}
	inode->isdevfs = 1;
	...
	//devfs_new_inode(dir, inode, mode, &error);
#if defined(_DEVFS_JOURN)
	...
	*/
	...
	if(!eidir->isjourn) {
	      ...
	      devfs_dbgv("%s: %s Dir journ init success i_ino %llu\n",
				__func__, dentry->d_name.name, dir->i_ino);
	}else {
	      devfs_dbgv("%s:%d dir journal already set to %d\n", 
				__FUNCTION__,__LINE__, eidir->isjourn);
	}

	trans = devfs_new_ino_trans_log(eidir, dir);
	if(!trans) {
		printk(KERN_ALERT "%s:%d Failed \n",
				__FUNCTION__, __LINE__);
		goto mknod_err;	
	...
	if(init_journal(inode)) {
        ...
        devfs_dbgv( "%s: %s mknode journal success i_ino %llu, isjourn %d, ei %lu\n",
	...
	if (inode) {
		...
		if(!ei->dentry_tree_init) {
			...
			devfs_dbgv("%s: Initializing inode %llu \n", __func__, ino);
		}
		error = devfs_add_dentry(dentry, ino, 0, 0, &tail);
		if (error){
			printk(KERN_ALERT "%s:%d devfs_add_dentry Failed\n", 
			__FUNCTION__,__LINE__);	
		}
#endif
...
#if defined(_DEVFS_JOURN)
	if(eidir->isjourn)
		devfs_commit_transaction(eidir, trans);

        devfs_dbgv("%s: %s mknode journal success i_ino %llu, isjourn %d\n",
...
int devfs_mkdir(struct inode * dir, struct dentry * dentry, 
	umode_t mode)
{

	int retval = devfs_mknod(dir, dentry, mode | S_IFDIR, 0);
	if (!retval)
		inc_nlink(dir);
#ifdef _DEVFS_DEBUG
	printk(KERN_ALERT "devfs_mkdir called \n");
#endif
	return retval;
}

int devfs_create(struct inode *dir, struct dentry *dentry, 
	umode_t mode, bool excl)
{
#ifdef _DEVFS_DEBUG
	printk(KERN_ALERT "devfs_create called \n");
#endif
	return devfs_mknod(dir, dentry, mode | S_IFREG, 0);
}

int devfs_symlink(struct inode * dir, struct dentry *dentry, 
	const char * symname)
{
	struct inode *inode;
	int error = -ENOSPC;
	...
	inode = devfs_get_inode(dir->i_sb, dir, S_IFLNK|S_IRWXUGO, 0);
	if (inode) {
...
int devfs_rmdir(struct inode *dir, struct dentry *dentry)
{
...
int devfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	devfs_dbgv("%s: inode %lu, dir %lu\n", __func__,
				inode->i_ino, dir->i_ino);
	retval = devfs_remove_dentry(dentry, 0, 0, &pidir_tail);
	if (retval) {
		printk("%s: %s devfs_remove_dentry FAILED\n", 
				__func__, dentry->d_name.name);
	}
#endif
...
static struct kmem_cache *devfs_inode_cachep;
static struct kmem_cache *devfs_vfsinode_cachep;

inline static struct inode *alloc_vfs_inode(void){

        return kmem_cache_alloc(devfs_vfsinode_cachep, GFP_KERNEL);
}
...
static struct inode *devfs_alloc_inode(struct super_block *sb)
{
	...
	devfs_dbgv("devfs_inode size %lu, vfs_inode size %lu \n",
			sizeof(struct devfs_inode), sizeof(struct inode));
	...
	devfs_dbgv("%s:%d Success allocating inode\n",__FUNCTION__,__LINE__);

	ei->vfs_inode->devfs_host = 0;
	//ei->i_block_alloc_info = NULL;
	//ei->vfs_inode->i_version = 1;
	atomic64_set(&ei->vfs_inode->i_version, 1);
	return ei->vfs_inode;
#else
	//ei->vfs_inode.i_version = 1;
	atomic64_set(&ei->vfs_inode.i_version, 1);
		...
		struct inode *inode, struct dentry *dentry)
{
	...
	if(!inode) {
	 ...
	 BUG_ON(!sbi->i_host_slab);
         h_inode = (struct inode *)devfs_slab_alloc(sbi, sbi->i_host_slab, 
	...
	h_inode->i_op = &devfs_file_inode_operations;
	h_inode->i_fop = &devfs_file_operations;
	h_inode->devfs_host = 1;
	...
	kmem_cache_free(devfs_vfsinode_cachep, inode);

	//if(ei->vfs_inode){
          //      inode_init_once(ei->vfs_inode);
        //}
	//devfs_dbgv("%s:%d Alloc inode offload path success\n",__FUNCTION__,__LINE__);
        //ei->i_block_alloc_info = NULL;
        //ei->vfs_inode->i_version = 1;
	return ei->vfs_inode;
...
struct inode *inode_ld_from_host(struct super_block *sb, 
		struct inode *h_inode)
{
        struct devfs_inode *ei = DEVFS_I(h_inode);
	struct devfs_sb_info *sbi = DEVFS_SB(sb);
	struct inode *inode = NULL;

        if(!ei) {
                printk(KERN_ALERT "%s:%d FAILED devfs_inode \n",
                         __FUNCTION__,__LINE__);
                goto skip_ino_load;
        }
	...
	h_inode->devfs_flags = HOST_INODE_DELETED;
	...
	//Disable the inode devfs_host flag now that we will use 
	//device memory
	ei->vfs_inode->devfs_host = 0;
...
static void devfs_i_callback(struct rcu_head *head)
{
	struct devfs_inode *ei;
	...
	//So, ideally freeing should not cause any concern
        kmem_cache_free(devfs_vfsinode_cachep, inode);
#endif
        kmem_cache_free(devfs_inode_cachep, ei);
...
static void devfs_delete_inodes(struct inode *inode)
{
	struct devfs_inode *ei;
...
unsigned int release_cache_pages(struct devfs_inode *ei, struct inode *inode)
{
	void **slot;
	unsigned int ret = 0;
...
#else
			if(radix_tree_delete(&mapping->i_pages, index) == NULL)
#endif
...
void devfs_evict_inode(struct inode *inode)
{
...
void devfs_destroy_inode(struct inode *inode)
{
	struct devfs_inode *ei;
	//struct dentry *dentry = NULL;

	printk(KERN_ALERT "devfs_destroy_inode getting called \n");
...
//PMFS CAUTION COMMENT
...
#endif
	devfs_dbgv("Calling devfs_destroy_inode \n");
	...
	devfs_delete_inodes(inode);
#else
        call_rcu(&inode->i_rcu, devfs_i_callback);
#endif
	devfs_dbgv("Finished devfs_destroy_inode \n");
}

/************ DevFS inode info allocation and release *****************/

struct devfs_mount_opts {
	umode_t mode;
};
...
struct devfs_fs_info {
	struct devfs_mount_opts mount_opts;
};

static int devfs_parse_options(char *data, struct devfs_mount_opts *opts)
{
...
#if defined(_DEVFS_BITMAP)
struct buffer_head *devfs_sb_bread(struct super_block *s, unsigned long block)
{
...
static int devfs_fill_super_extend(struct super_block *s, void *data, int silent)
{
	struct buffer_head *bh;
	struct buffer_head **map;
	struct devfs_super_block *ms;
	unsigned long i, block;
	struct inode *root_inode;
	struct devfs_sb_info *sbi;
	int ret = -EINVAL;

	sbi = kzalloc(sizeof(struct devfs_sb_info), GFP_KERNEL);
	if (!sbi)
		return -ENOMEM;
	...
	if (!(bh = devfs_sb_bread(s, 1)))
		goto out_bad_sb;

	ms = (struct devfs_super_block *) bh->b_data;
	sbi->s_ms = ms;
	sbi->s_sbh = bh;
	...
	for (i=0 ; i < sbi->s_imap_blocks ; i++) {
		if (!(sbi->s_imap[i]=devfs_sb_bread(s, block)))
			goto out_no_bitmap;
		block++;
	}

	for (i=0 ; i < sbi->s_zmap_blocks ; i++) {
		if (!(sbi->s_zmap[i]=devfs_sb_bread(s, block)))
			goto out_no_bitmap;
		block++;
	}

	devfs_set_bit(0,sbi->s_imap[0]->b_data);
	devfs_set_bit(0,sbi->s_zmap[0]->b_data);

	/* Apparently minix can create filesystems that allocate more blocks for
	 * the bitmaps than needed.  We simply ignore that, but verify it didn't
	 * create one with not enough blocks and bail out if so.
	 */
	block = devfs_blocks_needed(sbi->s_ninodes, s->s_blocksize);
...
{
	return pmfs_fill_super(sb, data, silent);
}

#if 0
int devfs_fill_super(struct super_block *sb, void *data, int silent)
{
	struct devfs_fs_info *fsi;
	struct inode *inode;
	int err;
	...
	devfs_parse_options2(data, sbi, 0);
	...
	if(!sbi->initsize)	
       ...
       /* Init a new pmfs instance */
        if (initsize) {
                root_pi = devfs_init(sb, initsize);

                if (IS_ERR(root_pi)) {
                      printk(KERN_ALERT "devfs_init failed \n");
                }
...
#ifdef _DEVFS_DEBUG
	printk(KERN_ALERT "devfs_fill_super called \n");
#endif
	save_mount_options(sb, data);

	fsi = kzalloc(sizeof(struct devfs_fs_info), GFP_KERNEL);
	sb->s_fs_info = fsi;
	if (!fsi)
		return -ENOMEM;

	err = devfs_parse_options(data, &fsi->mount_opts);
	if (err)
		return err;
...
#if defined(_DEVFS_BITMAP)
	devfs_fill_super_extend(sb, NULL, 0);
#endif
	...
	devfs_set_default_opts(sbi);

        if (devfs_alloc_block_free_lists(sb)) {
		return -ENOMEM;
        }
...
#endif
	inode = devfs_get_inode(sb, NULL, S_IFDIR | fsi->mount_opts.mode, 0);
...
struct dentry *devfs_mount(struct file_system_type *fs_type,
	int flags, const char *dev_name, void *data)
{
	printk(KERN_ALERT "devfs_mount called \n");
...
{
	//kfree(sb->s_fs_info);
	printk(KERN_ALERT "devfs_kill_sb called \n");
...
static void init_devfs_inode_once(void *foo)
{
        struct devfs_inode *ei = (struct devfs_inode *) foo;
...
int init_inodecache(void)
{
        devfs_inode_cachep = kmem_cache_create("devfs_inode_cache",
                                             sizeof(struct devfs_inode),
                                             0, (SLAB_RECLAIM_ACCOUNT|
                                                SLAB_MEM_SPREAD),
                                             init_devfs_inode_once);
        if (devfs_inode_cachep == NULL) {
		printk(KERN_ALERT "FAILED %s:%d\n",__FUNCTION__,__LINE__);
                return -ENOMEM;
	}
        devfs_vfsinode_cachep = kmem_cache_create("devfs_vfsinode_cachep",
                                             sizeof(struct inode),
                                             0, (SLAB_RECLAIM_ACCOUNT|
                                                SLAB_MEM_SPREAD),
                                             init_inode_once);
        if (devfs_vfsinode_cachep == NULL) {
		printk(KERN_ALERT "FAILED %s:%d\n",__FUNCTION__,__LINE__);
                return -ENOMEM;
	}
        return 0;
...
void destroy_inodecache(void)
{
        ...
        kmem_cache_destroy(devfs_vfsinode_cachep);
}


#if 0 //def _DEVFS_NOVA_BASED

int devfs_init_inode_inuse_list(struct super_block *sb)
{
        struct devfs_sb_info *sbi = DEVFS_SB(sb);
        struct devfs_range_node *range_node;
        struct inode_map *inode_map;
        ...
        for (i = 0; i < sbi->cpus; i++) {
                inode_map = &sbi->inode_maps[i];
                range_node = devfs_alloc_inode_node(sb);
                if (range_node == NULL)
                ...
                ret = devfs_insert_inodetree(sbi, range_node, i);
                if (ret) {
                        printk("%s failed\n", __func__);
                        devfs_free_inode_node(sb, range_node);
                        return ret;
                }
                inode_map->num_range_node_inode = 1;
                inode_map->first_inode_range = (struct devfs_range_node *)range_node;
        }
...
int devfs_init_inode_table(struct super_block *sb)
{
        struct devfs_sb_info *sbi = DEVFS_SB(sb);
        struct inode_table *inode_table;
        struct devfss_inode *pi = devfs_get_inode_by_ino(sb, DEVFS_INODETABLE_INO);
	...
	printk("%s:%d Enter\n", __FUNCTION__,__LINE__);	
        ...
        for (i = 0; i < sbi->cpus; i++) {
                ...
                inode_table = devfs_get_inode_table(sb, i);
			...
			printk("%s:%d Failed\n", __FUNCTION__,__LINE__);
                ...
                allocated = devfs_new_log_blocks(sb, pi, &blocknr, 1, 1);
                if (allocated != 1 || blocknr == 0) {
			printk("%s:%d Failed\n", __FUNCTION__,__LINE__);
                        return -ENOSPC;
		}

                block = devfs_get_block_off(sb, blocknr, pi->i_blk_type);
		...
		printk("%s:%d Block num %lu, Block off %llu\n", 
			__FUNCTION__,__LINE__, blocknr, block);
                ...
                devfs_flush_buffer(inode_table, CACHELINE_SIZE, 0);
        }
...
static int devfs_alloc_unused_inode(struct super_block *sb, int cpuid,
	unsigned long *ino)
{
	struct devfs_sb_info *sbi = DEVFS_SB(sb);
	struct inode_map *inode_map;
	struct devfs_range_node *i, *next_i;
	...
	if (!next) {
		...
		next_i = container_of(next, struct devfs_range_node, node);
	...
	if (next_i && new_ino == (next_range_low - 1)) {
		...
		devfs_free_inode_node(sb, next_i);
...
static int devfs_free_inuse_inode(struct super_block *sb, unsigned long ino)
{
	struct devfs_sb_info *sbi = DEVFS_SB(sb);
	struct inode_map *inode_map;
	struct devfs_range_node *i = NULL;
	struct devfs_range_node *curr_node;
	int found = 0;
	...
	found = devfs_search_inodetree(sbi, ino, &i);
	...
	if ((internal_ino > i->range_low) && (internal_ino < i->range_high)) {
		...
		curr_node = devfs_alloc_inode_node(sb);
		...
		ret = devfs_insert_inodetree(sbi, curr_node, cpuid);
		if (ret) {
			devfs_free_inode_node(sb, curr_node);
			goto err;
		}

#if defined(_DEVFS_DEBUG)
		printk("devfs_insert_inodetree %lu - %lu\n", 
...
static int devfs_free_contiguous_log_blocks(struct super_block *sb,
	struct devfss_inode *pi, u64 head)
{
	//struct devfs_inode_log_page *curr_page;
	...
	while (curr_block) {
		...
		//curr_page = (struct devfs_inode_log_page *)devfs_get_block(sb,
		//					curr_block);

		blocknr = devfs_get_blocknr(sb, le64_to_cpu(curr_block),
				    btype);
		...
		if (start_blocknr == 0) {
			...
			if (blocknr == start_blocknr + num_free) {
				...
				/* A new start */
				devfs_free_log_blocks(sb, pi, start_blocknr,
							num_free);
				freed += num_free;
				start_blocknr = blocknr;
				num_free = 1;
	...
	if (start_blocknr) {
		devfs_free_log_blocks(sb, pi, start_blocknr, num_free);
		freed += num_free;
	}
...
void devfs_free_inode_log(struct super_block *sb, struct devfss_inode *pi)
{
        u64 curr_block;
        int freed = 0;
	 ...
	 //printk("%s: calling devfs_free_contiguous_log_blocks \n", 
	//	__func__);

        freed = devfs_free_contiguous_log_blocks(sb, pi, curr_block);
...
static int devfss_free_inode(struct inode *inode)
{
	...
	int err = 0;
	//struct devfs_inode *sih = DEVFS_I(inode);
	...
	printk("%s: Calling devfs_free_inode_log \n",__func__);
	devfs_free_inode_log(sb, pi);
#endif
	pi->i_blocks = 0;
	...
	sih->i_size = 0;
#endif
	err = devfs_free_inuse_inode(sb, pi->nova_ino);
...
int devfs_get_inode_address(struct super_block *sb, u64 ino,
	u64 *pi_addr, int extendable)
{
	...
	pi = devfs_get_inode_by_ino(sb, DEVFS_INODETABLE_INO);
	data_bits = devfs_blk_type_to_shift[pi->i_blk_type];
	num_inodes_bits = data_bits - DEVFS_INODE_BITS;

	cpuid = ino % sbi->cpus;
	internal_ino = ino / sbi->cpus;
	inode_table = devfs_get_inode_table(sb, cpuid);
	...
	for (i = 0; i < superpage_count; i++) {
		...
		curr_addr = (unsigned long)devfs_get_block(sb, curr);
		...
		if (curr == 0) {
			...
			allocated = devfs_new_log_blocks(sb, pi, &blocknr,
							1, 1);
			if (allocated != 1) {
				printk("devfs_new_log_blocks allocated %d\n", allocated);
				return allocated;
			}

			curr = devfs_get_block_off(sb, blocknr,
						pi->i_blk_type);
			*(u64 *)(curr_addr) = curr;
			devfs_flush_buffer((void *)curr_addr,
						DEVFS_INODE_SIZE, 1);
		}
	}
...
u64 devfss_new_devfs_inode(struct super_block *sb, u64 *pi_addr)
{
	struct devfs_sb_info *sbi = DEVFS_SB(sb);
	struct inode_map *inode_map;
	...
	ret = devfs_alloc_unused_inode(sb, map_id, &free_ino);
	if (ret) {
	...
	ret = devfs_get_inode_address(sb, free_ino, pi_addr, 1);
	if (ret) {
...
static void devfs_set_next_page_flag(struct super_block *sb, u64 curr_p)
{
        void *p;
        ...
        p = devfs_get_block(sb, curr_p);
        devfs_set_entry_type(p, NEXT_PAGE);
        //nova_flush_buffer(p, CACHELINE_SIZE, 1);
}


static int devfs_coalesce_log_pages(struct super_block *sb,
	...
	unsigned long num_pages)
{
	...
	if (prev_blocknr) {
		...
		curr_block = devfs_get_block_off(sb, prev_blocknr,
						DEVFS_BLOCK_TYPE_4K);
		curr_page = (struct devfs_inode_log_page *)
				devfs_get_block(sb, curr_block);
		next_page = devfs_get_block_off(sb, first_blocknr,
				DEVFS_BLOCK_TYPE_4K);
		devfs_set_next_page_address(sb, curr_page, next_page, 0);
	}

	next_blocknr = first_blocknr + 1;
	curr_block = devfs_get_block_off(sb, first_blocknr,
						DEVFS_BLOCK_TYPE_4K);
	curr_page = (struct devfs_inode_log_page *)
				devfs_get_block(sb, curr_block);
	for (i = 0; i < num_pages - 1; i++) {
		next_page = devfs_get_block_off(sb, next_blocknr,
				DEVFS_BLOCK_TYPE_4K);
		devfs_set_next_page_address(sb, curr_page, next_page, 0);
		curr_page++;
		next_blocknr++;
	...
	/* Last page */
	devfs_set_next_page_address(sb, curr_page, 0, 1);
	return 0;
}


/* Log block resides in NVMM */
int devfs_allocate_inode_log_pages(struct super_block *sb,
	...
	u64 *new_block)
{
	...
	allocated = devfs_new_log_blocks(sb, pi, &new_inode_blocknr,
					num_pages, 0);
	...
	while (num_pages) {
		...
		ret_pages += allocated;
		num_pages -= allocated;
		devfs_coalesce_log_pages(sb, prev_blocknr, new_inode_blocknr,
						allocated);
		prev_blocknr = new_inode_blocknr + allocated - 1;
	}

	*new_block = devfs_get_block_off(sb, first_blocknr,
						DEVFS_BLOCK_TYPE_4K);
...
static u64 devfs_extend_inode_log(struct super_block *sb, struct devfss_inode *pi,
	struct devfs_inode *sih, u64 curr_p)
{
	u64 new_block;
	int allocated;
	unsigned long num_pages;
	...
	if (curr_p == 0) {
		allocated = devfs_allocate_inode_log_pages(sb, pi,
					1, &new_block);
		if (allocated != 1) {
...
//		nova_print_inode_log_page(sb, inode);
		...
		//devfs_inodelog_fast_gc(sb, pi, sih, curr_p,
		//				new_block, allocated);
...
static u64 devfs_append_one_log_page(struct super_block *sb,
	struct devfss_inode *pi, u64 curr_p)
{
	...
	allocated = devfs_allocate_inode_log_pages(sb, pi, 1, &new_block);
	if (allocated != 1) {
	...
	if (curr_p == 0) {
		...
		curr_page = (struct devfs_inode_log_page *)
				devfs_get_block(sb, curr_block);
		devfs_set_next_page_address(sb, curr_page, new_block, 1);
	}
...
u64 devfs_get_append_head(struct super_block *sb, struct devfss_inode *pi,
	struct devfs_inode *sih, u64 tail, size_t size, int *extended)
{
	u64 curr_p;
	...
	if (curr_p == 0 || (is_last_entry(curr_p, size) &&
		...
		if (is_last_entry(curr_p, size))
			devfs_set_next_page_flag(sb, curr_p);

		if (sih) {

			devfs_dbgv("%s:%d\n",__FUNCTION__,__LINE__);
			curr_p = devfs_extend_inode_log(sb, pi, sih, curr_p);
		} else {

			devfs_dbgv("%s:%d\n",__FUNCTION__,__LINE__);
			curr_p = devfs_append_one_log_page(sb, pi, curr_p);
			/* For thorough GC */
			*extended = 1;
		}
