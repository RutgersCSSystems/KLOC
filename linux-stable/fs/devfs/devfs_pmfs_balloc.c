...
#if 1
//int __init devfs_init_blocknode_cache(void)
int devfs_init_blocknode_cache(void)
{
...
void pmfs_init_blockmap(struct super_block *sb, unsigned long init_used_size)
{
        ...
        printk(KERN_ALERT "%s:%d pmfs_init_blockmap\n",__FUNCTION__,__LINE__);

	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	//struct devfs_sb_info *sbi = DEVFS_SB(sb);
	if(!sbi) {
		printk(KERN_ALERT "%s:%d pmfs_init_blockmap failed\n",
				__FUNCTION__,__LINE__);
	}
	printk(KERN_ALERT "%s:%d pmfs_init_blockmap\n",__FUNCTION__,__LINE__);
	...
	devfs_dbgv("Success %s:%d block_low %lu block_high %lu num_free_blocks %lu\n", 
