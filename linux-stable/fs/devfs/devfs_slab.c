...
void *devfs_slab_alloc(struct devfs_sb_info *sbi, struct slab_chain *sch, 
...
int devfs_slab_test(struct devfs_sb_info *sbi)
{
    struct slab_chain s;
    ssize_t i = 0;
    ssize_t j = 0;
...
#if defined(_DEVFS_SLAB_USER)
	allocs[i] = devfs_slab_alloc(&sbi1, &s, &sbi1->i_host_addr, &sbi1->i_host_off);
#else
        allocs[i] = devfs_slab_alloc(sbi, &s, &sbi->i_host_addr, &sbi->i_host_off);
#endif
...
#if defined(_DEVFS_SLAB_USER)
	printf("devfs_slab_init finished allocation %d \n", i );
#else
    printk(KERN_ALERT "devfs_slab_init finished allocation %d \n", i );
#endif
...
struct slab_chain *devfs_slab_init(struct devfs_sb_info *sbi, ssize_t size)
{
...
struct slab_chain *devfs_slab_free(struct devfs_sb_info *sbi, struct slab_chain *s)
{
