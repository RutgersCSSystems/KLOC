	...
	struct pmfs_sb_info *sbi)
{
	...
	printk(KERN_ALERT "Checking DAX \n");
	...
	printk(KERN_ALERT "Checking fs_dax_get_by_host\n");
	...
	printk(KERN_ALERT "Checking dax_direct_access\n");
	...
	printk(KERN_ALERT "Setting DAX addr and returning %lu\n",  sbi->virt_addr);
...
enum {
	Opt_addr, Opt_bpi, Opt_init, Opt_size, Opt_jsize,
				      ...
				      unsigned long size)
{
	...
	pmfs_init_blockmap(sb, journal_data_start + sbi->jsize);
	printk(KERN_ERR "%s:%d pmfs_init_blockmap\n",__FUNCTION__,__LINE__);
#endif
	pmfs_dbg_verbose("journal meta start %llx data start 0x%llx, "
		"journal size 0x%x, inode_table 0x%llx\n", journal_meta_start,
		journal_data_start, sbi->jsize, inode_table_start);
...
#endif
	printk(KERN_ERR "%s:%d Before pmfs_init_inode_table\n",__FUNCTION__,__LINE__);

	if (pmfs_init_inode_table(sb) < 0)
		return ERR_PTR(-EINVAL);

	printk(KERN_ERR "%s:%d After pmfs_init_inode_table\n",__FUNCTION__,__LINE__);
...
int pmfs_fill_super(struct super_block *sb, void *data, int silent)
{
	struct pmfs_super_block *super;
...
int pmfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	printk(KERN_ALERT "Before pmfs_fill_super \n");
	...
	printk(KERN_ALERT "Before pmfs_parse_options \n");
	...
	printk(KERN_ALERT "After pmfs_parse_options \n");
	...
	if (sbi->phys_addr == (phys_addr_t)ULLONG_MAX) {
		printk(KERN_ALERT "After pmfs_fill_super %s:%d %llu\n",
	...
	if (initsize) {
		...
		printk(KERN_ERR "%s:%d Before pmfs_init\n",__FUNCTION__,__LINE__);
		root_pi = pmfs_init(sb, initsize);
		printk(KERN_ERR "%s:%d After pmfs_init\n",__FUNCTION__,__LINE__);
		if (IS_ERR(root_pi))
			goto out;
	...
	pmfs_dbg_verbose("checking physical address 0x%016llx for pmfs image\n",
		  (u64)sbi->phys_addr);
	printk(KERN_ERR "%s:%d Before pmfs_ioremap\n",__FUNCTION__,__LINE__);
...
static struct kmem_cache *devfs_range_node_cachep;
unsigned int devfs_dbgmask = 0;
...
int devfs_parse_options2(char *options, struct devfs_sb_info *sbi,
			       bool remount)
{
	char *p, *rest;
	substring_t args[MAX_OPT_ARGS];
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_dbgmask:
			if (match_int(&args[0], &option))
				goto bad_val;
			devfs_dbgmask = option;
			printk(KERN_ALERT "devfs_dbgmask=%d set? \n",devfs_dbgmask);
			break;
...
{
	devfs_range_node_cachep = kmem_cache_create("devfs_range_node_cache",
					sizeof(struct devfs_range_node),
					0, (SLAB_RECLAIM_ACCOUNT |
                                        SLAB_MEM_SPREAD), NULL);
	if (devfs_range_node_cachep == NULL)
		return -ENOMEM;
	return 0;
}
...
static void devfs_set_blocksize(struct super_block *sb, unsigned long size)
{
	int bits;
...
inline void devfs_set_default_opts(struct devfs_sb_info *sbi)
{
        //set_opt(sbi->s_mount_opt, HUGEIOREMAP);
        //set_opt(sbi->s_mount_opt, ERRORS_CONT);
				      ...
				      unsigned long size)
{
	...
	struct devfs_super_block *super;
	...
	if(sbi->vmmode) {
		...
		sbi->virt_addr = devfs_ioremap(sb, sbi->phys_addr, sbi->initsize, "devfsmeta");
		if(!sbi->virt_addr) {
	...
	devfs_dbgv("creating an empty devfs of size %lu\n", sbi->initsize);
...
alloc_virtaddr:
	...
	devfs_dbgv("Before _DEVFS_DENTRY_OFFLOAD %lu\n", sbi->virt_addr);
	...
	devfs_set_blocksize(sb, blocksize);
	blocksize = sb->s_blocksize;

	if (sbi->blocksize && sbi->blocksize != blocksize)
		sbi->blocksize = blocksize;

	/*if (!devfs_check_size(sb, size)) {
		devfs_dbg("Specified NOVA size too small 0x%lx.\n", size);
		return ERR_PTR(-EINVAL);
	}*/
	...
	super = devfs_get_super(sb);

	/* clear out super-block and inode table */
	devfs_memset_nt(super, 0, sbi->reserved_blocks * sbi->blocksize);
	super->s_size = cpu_to_le64(size);
	super->s_blocksize = cpu_to_le32(blocksize);
	super->s_magic = cpu_to_le32(DEVFS_SUPER_MAGIC);

	if(devfs_init_blockmap(sb, 0)) {
		printk("%s:%d Failed devfs_init_blockmap \n",
			__FUNCTION__,__LINE__);
	}

#if defined(_DEVFS_SLAB_ALLOC)
	if(sbi->i_host_addr) {
		sbi->i_host_slab = devfs_slab_init(sbi, sizeof(struct inode));
...
//#if  defined(_DEVFS_DENTRY_OFFLOAD) && defined(_DEVFS_SLAB_ALLOC)
#if defined(_DEVFS_SLAB_ALLOC)
	if(sbi->d_host_addr) {
		sbi->d_host_slab = devfs_slab_init(sbi, sizeof(struct dentry));
...
#if 0
	if (devfs_lite_journal_hard_init(sb) < 0) {
		printk(KERN_ERR "Lite journal hard initialization failed\n");
		return ERR_PTR(-EINVAL);
	}
#endif
	if (devfs_init_inode_inuse_list(sb) < 0)
		return ERR_PTR(-EINVAL);

	if (devfs_init_inode_table(sb) < 0) {
		printk("%s:%d Failed devfs_init_inode_table\n",
			__FUNCTION__,__LINE__);
		return ERR_PTR(-EINVAL);
	}
	...
	pi = (struct devfss_inode *)devfs_get_inode_by_ino
			(sb, DEVFS_BLOCKNODE_INO);

	pi->nova_ino = DEVFS_BLOCKNODE_INO;

	devfs_dbgv("%s:%d Afrer devfs_get_inode_by_ino\n",
			__FUNCTION__,__LINE__);
	...
	root_i = devfs_get_inode_by_ino(sb, DEVFS_ROOT_INO);
	...
	if(devfs_append_dir_init_entries(sb, root_i, 
				      ...
				      unsigned long size)
{
	...
	printk(KERN_ALERT "trying to execute ioremap size %lu "
	 ...
	 sbi->virt_addr = NULL;
#else
	sbi->virt_addr = devfs_ioremap(sb, sbi->phys_addr, size);
#endif
...
alloc_virtaddr:
	...
	printk("%s:%d Afrer devfs_get_inode_by_ino\n",
			__FUNCTION__,__LINE__);
...
inline void devfs_free_range_node(struct devfs_range_node *node)
{
	kmem_cache_free(devfs_range_node_cachep, node);
}

inline void devfs_free_blocknode(struct super_block *sb,
	struct devfs_range_node *node)
{
	devfs_free_range_node(node);
}

inline void devfs_free_inode_node(struct super_block *sb,
	struct devfs_range_node *node)
{
	devfs_free_range_node(node);
}
...
static inline
struct devfs_range_node *devfs_alloc_range_node(struct super_block *sb)
{
	struct devfs_range_node *p;
	p = (struct devfs_range_node *)
		kmem_cache_alloc(devfs_range_node_cachep, GFP_NOFS);
	return p;
}

inline struct devfs_range_node *devfs_alloc_blocknode(struct super_block *sb)
{
	return devfs_alloc_range_node(sb);
}

inline struct devfs_range_node *devfs_alloc_inode_node(struct super_block *sb)
{
	return devfs_alloc_range_node(sb);
}
...
{
	struct devfs_inode_info *vi;

	vi = kmem_cache_alloc(devfs_inode_cachep, GFP_NOFS);
	if (!vi)
		return NULL;
...
{
	struct inode *inode = container_of(head, struct inode, i_rcu);
	struct devfs_inode_info *vi = NOVA_I(inode);

	printk("%s: ino %lu\n", __func__, inode->i_ino);
	kmem_cache_free(devfs_inode_cachep, vi);
}
...
static void init_once(void *foo)
{
	struct devfs_inode_info *vi = foo;
