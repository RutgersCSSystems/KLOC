...
struct devfs_dentry *devfs_find_dentry(struct super_block *sb,
	struct devfss_inode *pi, struct inode *inode, const char *name,
	unsigned long name_len)
{
	struct devfs_inode *si = DEVFS_I(inode);
	struct devfs_dentry *direntry;
	unsigned long hash;
...
static int devfs_insert_dir_radix_tree(struct super_block *sb,
	struct devfs_inode *si, const char *name,
	int namelen, struct devfs_dentry *direntry)
{
	unsigned long hash;
	int ret;

	hash = BKDRHash(name, namelen);
	devfs_dbgv("%s: insert %s hash %lu\n", __func__, name, hash);
...
static int devfs_check_dentry_match(struct super_block *sb,
	struct devfs_dentry *dentry, const char *name, int namelen)
{
...
static int devfs_remove_dir_radix_tree(struct super_block *sb,
	struct devfs_inode *si, const char *name, int namelen,
	int replay)
{
	struct devfs_dentry *entry;
	unsigned long hash;
	...
	if (replay == 0) {
		...
		//if (entry->ino == 0 || 
		if (entry->invalid ||
		    devfs_check_dentry_match(sb, entry, name, namelen)) {
...
void devfs_delete_dir_tree(struct super_block *sb,
	struct devfs_inode *si)
{
	struct devfs_dentry *direntry;
	unsigned long pos = 0;
	struct devfs_dentry *entries[FREE_BATCH];
	int nr_entries;
	int i;
	void *ret;
...
/*
 * Append a devfs_dentry to the current devfss_inode_log_page.
 * Note unlike append_file_write_entry(), this method returns the tail pointer
 * after append.
 */
static u64 devfs_append_dir_inode_entry(struct super_block *sb,
...
{
	struct devfs_inode *si = DEVFS_I(dir);
	struct devfs_dentry *entry;
	u64 curr_p;
	size_t size = de_len;
	int extended = 0;
	...
	curr_p = devfs_get_append_head(sb, pidir, si, tail, size, &extended);
	if (curr_p == 0)
		BUG();

	entry = (struct devfs_dentry *)devfs_get_block(sb, curr_p);
	...
	devfs_dbgv("dir entry @ 0x%llx: ino %llu, entry len %u, "
...
/* Append . and .. entries */
int devfs_append_dir_init_entries(struct super_block *sb,
	struct devfss_inode *pi, u64 self_ino, u64 parent_ino)
{
	int allocated;
	u64 new_block;
	u64 curr_p;
	struct devfs_dentry *de_entry;
	...
	de_entry = (struct devfs_dentry *)devfs_get_block(sb, new_block);
	...
	de_entry = (struct devfs_dentry *)((char *)de_entry +
	...
	devfs_update_tail(pi, curr_p);

	devfs_dbgv("%s: devfs_update_tail %llu\n",
 ...
 */
int devfs_add_dentry(struct dentry *dentry, u64 ino, int inc_link,
	u64 tail, u64 *new_tail)
{
	...
	devfs_dbgv("%s: dir %lu new inode %llu\n",
	...
	//devfs_dbgv("%s: %s %lu tail %llu, head %llu\n", __func__, 
	...
	curr_entry = devfs_append_dir_inode_entry(sb, pidir, dir, ino,
				dentry,	loglen, tail, inc_link,
				&curr_tail);

	direntry = (struct devfs_dentry *)devfs_get_block(sb, curr_entry);
	ret = devfs_insert_dir_radix_tree(sb, si, name, namelen, direntry);
	*new_tail = curr_tail;
	devfs_dbgv("%s: %s curr_tail %llu \n", __func__, name, curr_tail);
	//NOVA_END_TIMING(add_dentry_t, add_dentry_time);

	if(!devfs_find_dentry(sb, pidir, dir, name, namelen)){
		devfs_dbgv("%s: Failed to locate %s \n", __func__, name);
	}else {
		devfs_dbgv("%s: Found entry %s \n", __func__, name);
	}
 ...
 * already been logged for consistency
 */
int devfs_remove_dentry(struct dentry *dentry, int dec_link, u64 tail,
	u64 *new_tail)
{
	struct inode *dir = dentry->d_parent->d_inode;
	struct super_block *sb = dir->i_sb;
	...
	curr_entry = devfs_append_dir_inode_entry(sb, pidir, dir, 0,
				dentry, loglen, tail, dec_link, &curr_tail);
	*new_tail = curr_tail;

	devfs_remove_dir_radix_tree(sb, si, entry->name, entry->len, 0);
...
inline int nova_replay_add_dentry(struct super_block *sb,
	struct devfs_inode *si, struct devfs_dentry *entry)
{
	if (!entry->name_len)
		return -EINVAL;

	printk("%s: add %s\n", __func__, entry->name);
	return devfs_insert_dir_radix_tree(sb, si,
...
{
	printk("%s: remove %s\n", __func__, entry->name);
	devfs_remove_dir_radix_tree(sb, si, entry->name,
					entry->name_len, 1);
	return 0;
}

static inline void nova_rebuild_dir_time_and_size(struct super_block *sb,
	struct devfss_inode *pi, struct devfs_dentry *entry)
{
	if (!entry || !pi)
		return;
...
int nova_rebuild_dir_inode_tree(struct super_block *sb,
	...
	struct devfs_inode *si)
{
	struct devfs_dentry *entry = NULL;
	...
	while (curr_p != pi->log_tail) {
		...
		addr = (void *)devfs_get_block(sb, curr_p);
	...
	curr_p &= PAGE_MASK;
	curr_page = (struct devfss_inode_log_page *)devfs_get_block(sb, curr_p);
	while ((next = curr_page->page_tail.next_page) != 0) {
		si->log_pages++;
		curr_p = next;
		curr_page = (struct devfss_inode_log_page *)
			devfs_get_block(sb, curr_p);
	}
...
static int nova_readdir(struct file *file, struct dir_context *ctx)
{
	...
	struct devfss_inode *pidir;
	struct devfs_inode *si = DEVFS_I(inode);
	struct devfs_inode *si = &si->header;
	...
	do {
		...
		for (i = 0; i < nr_entries; i++) {
			...
			ret = devfs_get_inode_address(sb, ino, &pi_addr, 0);
			if (ret) {
			...
			child_pi = devfs_get_block(sb, pi_addr);
...
static int nova_readdir(struct file *file, struct dir_context *ctx)
{
	...
	struct devfs_dentry *entry = NULL;
	struct devfs_dentry *prev_entry = NULL;
	unsigned short de_len;
	u64 pi_addr;
	unsigned long pos = 0;
	ino_t ino;
