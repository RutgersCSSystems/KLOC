...
__do_page_cache_readahead(struct address_space *mapping, struct file *filp,
			...
			unsigned long lookahead_size, struct devfs_inode *ei)
{
 ...
 */
unsigned long
devfs_ondemand_readahead(struct address_space *mapping,
		   struct file_ra_state *ra, struct file *filp,
		   bool hit_readahead_marker, pgoff_t offset,
		   unsigned long req_size, struct devfs_inode *ei)
{
 ...
 */
void
devfs_read_page(struct address_space *mapping,
	struct file_ra_state *ra, struct file *filp,
	...
	unsigned long req_size)
{
	...
	/* do read-ahead */
	devfs_ondemand_readahead(mapping, ra, filp, true, offset, req_size, ei);
}
EXPORT_SYMBOL_GPL(devfs_read_page);


/**
 * devfs_page_cache_async_readahead - file readahead for marked pages
 * @mapping: address_space which holds the pagecache and I/O vectors
 * @ra: file_ra_state which holds the readahead state
 * @filp: passed on to ->readpage() and ->readpages()
 * @page: the page at @offset which has the PG_readahead flag set
 ...
 *            pagecache pages
 *
 * devfs_page_cache_async_readahead() should be called when a page is used which
 * has the PG_readahead flag; this is a marker to suggest that the application
 * has used up enough of the readahead window that we should start pulling in
 * more pages.
 */
void
devfs_page_cache_async_readahead(struct address_space *mapping,
                           struct file_ra_state *ra, struct file *filp,
                           struct page *page, pgoff_t offset,
                           unsigned long req_size)
{
...
EXPORT_SYMBOL_GPL(devfs_page_cache_async_readahead);
 ...
 */
int devfs_add_page_cache_locked(struct page *page, struct address_space *mapping,
                pgoff_t offset, gfp_t gfp_mask, struct devfs_inode *ei)
{
        int error;
	struct inode *inode = mapping->host;	
...
EXPORT_SYMBOL(devfs_add_page_cache_locked);

/*
 * Like add_to_page_cache_locked, but used to add newly allocated pages:
 * the page is new, so we can just run __SetPageLocked() against it.
 */
static inline int devfs_add_to_page_cache(struct page *page,
		...
		struct devfs_inode *ei)
{
        ...
        error = devfs_add_page_cache_locked(page, mapping, offset, gfp_mask, ei);
        if (unlikely(error))
                __ClearPageLocked(page);
        return error;
}

int devfs_add_page_lru(struct page *page, struct address_space *mapping,
                          pgoff_t offset, gfp_t gfp_mask, struct devfs_inode *ei)
{
        int ret;

        ret = devfs_add_to_page_cache(page, mapping, offset, gfp_mask, ei);
        if (ret == 0)
                lru_cache_add_file(page);

        return ret;
}
EXPORT_SYMBOL_GPL(devfs_add_page_lru);


struct page *devfs_get_cache_page(struct address_space *mapping,
	pgoff_t index, unsigned flags, struct devfs_inode *ei, 
	struct inode *inode)
{
        int status;
        gfp_t gfp_mask;
...
#if defined(_DEVFS_NOVA_BASED)
        sb = inode->i_sb;
        //devfs_new_data_blocks(sb, NULL, &blocknr, 1, start_blk, 0, 0);
#endif
	...
	if(!page) {
        ...
        status = devfs_add_page_lru(page, mapping, index,
...
EXPORT_SYMBOL(devfs_get_cache_page);
 ...
 */
struct page *devfs_find_get_page(struct address_space *mapping, pgoff_t offset, 
					struct devfs_inode *ei)
{
        void **pagep;
        struct page *page;
...
EXPORT_SYMBOL(devfs_find_get_page);
#endif
