...
int devfs_run_test(nvme_cmdrw_t *cmdrw);
...
#ifdef _DEBUG
	devfs_dbgv( "%s:%d success file create %d name %s \n", 
		...
		struct devfs_fstruct *rd)
{
	...
	if ((opc == nvme_cmd_write) || (opc == nvme_cmd_append)) {
        	...
        	retval =  devfs_direct_write (rd,(void *)cmd, append);
#else
...
wait_for_task:
	...
	devfs_dbgv("%s:%d compressd %lu, input %lu g_compress_arg.compressd %lu\n",
	...
	devfs_dbgv("%s:%d compress_func success %zu\n",
...
int perform_compress(int fd, nvme_cmdrw_t *cmdrw, unsigned int fno){
	...
	int err = 0;
	int newfd = -1;
	struct devfs_fstruct *newrd = NULL;
...
#ifdef _DEBUG
	devfs_dbgv( "%s:%d File size isize %zu pos %llu\n", 
...
//This is not defined. Remove this ifdef after testing
	rdbytes = devfs_read(fp, (char __user *)p, isize, &fpos);
#else
	isize = 4096;
	rdbytes = isize;
#endif
	...
	rdbytes = vfio_devfs_io_read (oldrd, cmdrw, 1);*/
	if(rdbytes != isize) {
...
#ifdef _DEBUG
	devfs_dbgv( "%s:%d File read bytes %zu memloc %lu \n", 
		__FUNCTION__, __LINE__,rdbytes, (unsigned long)p);
#endif
	...
	devfs_dbgv("g_compressd addr %lu compress_len %u\n", 
...
#endif
#ifdef _DEBUG
	devfs_dbgv( "%s:%d Compression success %zu\n",
...
int devfs_snappy_test(struct devfs_fstruct *rd, nvme_cmdrw_t *cmdrw,
	...
	u8 *t_finish;
		...
		if(test_snappy_init_env()) 
			goto err_devfs_snappy_test;			

                g_snappy_init = 1;

                devfs_dbgv("%s:%d test_snappy_init_env  success\n",
                 __FUNCTION__, __LINE__);
        }
	...
	devfs_dbgv( "Starting %s:%d for %d blocks and  "
	...
	for (fidx=0; fidx < numfiles; fidx++) {	
...
#ifdef _DEBUG
                devfs_dbgv( "%s:%d create_file returns %d\n",
                          __FUNCTION__, __LINE__, fd);
#endif
		...
		/*devfs_flush_buffer(taskargs[cpuidx], 
...
int devfs_rwtest(struct devfs_fstruct *rd, nvme_cmdrw_t *cmdrw,
		size_t sz, int fd, int append){
...
#if defined(_DEVFS_SNAPPY_TEST)
	devfs_snappy_test(rd, cmdrw, sz, fd, append);
#endif
...
int devfs_thread_append(void *data){
...
#else
			if ((opc == nvme_cmd_write) || (opc == nvme_cmd_append))
	                        rdbytes =  devfs_direct_write (newrd,(void *)cmdrw, append);

			//Read not supported yet
			/*else if(opc == nvme_cmd_read) {
				rdbytes = vfio_devfs_io_read (rd, cmdrw, 1);
			}*/
#endif
		}
...
int devfs_run_test(nvme_cmdrw_t *cmdrw){
	...
	//rwthrd = kthread_run(devfs_thread_append, &task, "rwthread");
	devfs_thread_append(&task);
#if 0
	if (!rwthrd) {
