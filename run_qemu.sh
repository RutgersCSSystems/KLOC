...
e2fsck -yf qemu-image-fresh.img

$OFFLOADBASE/scripts/umount_qemu.sh
sleep 1
#Launching QEMU
...
sudo qemu-system-x86_64 -kernel $KERNEL/vmlinuz-$VER -hda $QEMU_IMG_FILE -append "root=/dev/sda rw console=tty0 console=ttyS0,9600n8" -m $QEMUMEM -smp cores=32 -cpu host -enable-kvm -device e1000,netdev=net0 -netdev user,id=net0,hostfwd=tcp::5555-:22 -net nic -nographic -serial mon:stdio -numa node,nodeid=0,cpus=$CORE1,mem=$QEMUNODE1 -numa node,nodeid=1,cpus=$CORE2,mem=$QEMUNODE2
exit
