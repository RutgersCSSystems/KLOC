...
#ifdef _USE_HETERO_PG_FLAG	
		/* Don't have to migrate slow memory pages? */
...
/*
 * Walk through HETERO small rbtree list and collect pages to be migrated.
 *
 * If pages found in a given range are on a set of nodes (determined by
 * @nodes and @flags,) it's isolated and queued to the pagelist which is
 * passed via @private.)
 */
static int
add_rbtree_small(struct mm_struct *mm, unsigned long start, unsigned long end,
		nodemask_t *nodes, unsigned long flags,
		struct list_head *pagelist, unsigned int *cnt)
{
	struct queue_pages qp = {
 ...
 */
static int
add_rbtree_large(struct mm_struct *mm, unsigned long start, unsigned long end,
		nodemask_t *nodes, unsigned long flags,
		struct list_head *pagelist, unsigned int *cnt)
{
	struct queue_pages qp = {
