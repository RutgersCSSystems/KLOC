...
#if 0
void devfs_free_block(struct inode *inode, unsigned long block)
{
...
static inline int devfs_find_first_zero_bit(const void *vaddr, unsigned size)
{
...
#if 0
int devfs_new_block(struct inode * inode)
{
	...
	for (i = 0; i < sbi->s_zmap_blocks; i++) {
		...
		spin_lock(&bitmap_lock);
		j = devfs_find_first_zero_bit(bh->b_data, bits_per_zone);
		if (j < bits_per_zone) {
...
unsigned long devfs_count_free_blocks(struct super_block *sb)
{
	struct devfs_sb_info *sbi = devfs_sb(sb);
...
struct devfs_inode *
devfs_raw_inode(struct super_block *sb, ino_t ino, struct buffer_head **bh)
{
	int block;
	struct devfs_sb_info *sbi = devfs_sb(sb);
	struct devfs_inode *p;
...
static void devfs_clear_inode(struct inode *inode)
{
	...
	if (1) {
		struct devfs_inode *raw_inode;
		raw_inode = devfs_raw_inode(inode->i_sb, inode->i_ino, &bh);
...
int devfs_free_inode(struct inode * inode)
{
	struct super_block *sb = inode->i_sb;
	struct devfs_sb_info *sbi = devfs_sb(inode->i_sb);
	struct buffer_head *bh;
	...
	devfs_clear_inode(inode);	/* clear on-disk copy */
#endif
	...
	clear_bit(bit, (volatile void *)bh->b_data);
...
int devfs_set_sb_bit(struct super_block *sb)
{
	struct devfs_sb_info *sbi = devfs_sb(sb);
	struct buffer_head * bh;
...
#endif
	set_bit(j, ( volatile void *)bh->b_data);
	spin_unlock(&bitmap_lock);
	//mark_buffer_dirty(bh);
...
struct inode* 
devfs_new_inode(struct super_block *sb, struct inode *inode, 
...
#if 0
	if (devfs_test_and_set_bit(j, bh->b_data)) {	/* shouldn't happen */
		spin_unlock(&bitmap_lock);
...
unsigned long devfs_count_free_inodes(struct super_block *sb)
{
	struct devfs_sb_info *sbi = devfs_sb(sb);
